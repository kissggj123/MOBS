...
namespace MBS {
	...
	/// <summary>
	/// The main MBS form, supporting multiple tabs.
	...
	internal partial class MainForm : Form {
        ...
        public static string UserAgent = "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) MOBS/1.0.0.100 Chrome/47.0.2526.73 Safari/537.36";
        public static string HomepageURL = "about:blank";
        public static string DownloadsURL = "MBS://storage/downloads.html";
		public static string FileNotFoundURL = "MBS://storage/errors/notFound.html";
		public static string CannotConnectURL = "MBS://storage/errors/cannotConnect.html";
		...
		private void InitAppIcon() {
			...
			Icon = new Icon(GetResourceStream("MBS.ico"), new Size(32, 32));
		}
		...
		public Stream GetResourceStream(string filename, bool withNamespace = true) {
			try {
				return assembly.GetManifestResourceStream("MBS.Resources." + filename);
		...
		private void InitBrowser() {
			...
			settings.RegisterScheme(new CefCustomScheme {
				SchemeName = "Menglolita Brwoser S",
			...
			settings.CachePath = GetAppDir("MBOSCache");
		...
		private static string GetAppDir(string name) {
            string winXPDir = AppDomain.CurrentDomain.BaseDirectory;
            if (Directory.Exists(winXPDir))
            {
                return winXPDir + Branding + @"\" + name + @"\";
            }
            return AppDomain.CurrentDomain.BaseDirectory + Branding + @"\" + name + @"\";
		...
		private void LoadURL(string url) {
			...
			if (urlLower == "localhost") {
				...
				if (!(urlLower.StartsWith("http") || urlLower.StartsWith("MBS"))) {
					if (outUri == null || outUri.Scheme != Uri.UriSchemeFile) newUrl = "http://" + url;
				}

				if (urlLower.StartsWith("MBS:") ||
		...
		private bool IsBlankOrSystem(string url) {
			return (url == "" || url.BeginsWith("about:") || url.BeginsWith("chrome:") || url.BeginsWith("MBS:"));
		}
		...
		private void OnTabsChanged(TabStripItemChangedEventArgs e) {
			...
			if (e.ChangeType == FATabStripItemChangeTypes.SelectionChanged) {
				if (TabPages.SelectedItem == tabStripAdd) {
					...
					SetFormTitle(browser.Tag.ConvertToString() ?? "新标签页");
		...
		private void bDownloads_Click(object sender, EventArgs e) {
            ...
            MessageBox.Show("下载功能正在努力开发中", "还没做下载功能");
		}
		...
		private void MainForm_FormClosing(object sender, FormClosingEventArgs e) {
			...
			if (DownloadsInProgress()) {
				if (MessageBox.Show("正在努力下载，真的要退出？", "退出吗？", MessageBoxButtons.YesNo, MessageBoxIcon.Question) != DialogResult.Yes) {
					e.Cancel = true;
					return;
				}
			}
        ...
        private void 设置为主页ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            s.Homepage = TxtURL.Text;
            MessageBox.Show(string.Format("你已成功设置 {0} 为你的新主页", TxtURL.Text), "设置主页成功", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }

        private void 关于MOBSToolStripMenuItem_Click(object sender, EventArgs e)
        {
